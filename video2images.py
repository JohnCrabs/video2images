...
def video2img(path: str, fps=0, nameIndex=0, imgFormat="jpg"):
    ...
    while success:
        if currFrameCounter % fps == 0:
            cv2.imwrite("outputData/%03d." % nameIndex + imgFormat, frame)  # save frame as JPEG file
...
def videoINfolder2image(folderPath: str, fps = 0, imgFormat="jpg"):
    videoFiles = []
    fileCounter = 0
    ...
    for f in videoFiles:
        ...
        success, frameIndex = video2img(f, fps, frameIndex, imgFormat)

#video2img(pathVideo)
videoINfolder2image(pathVideo)